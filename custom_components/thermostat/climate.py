...
class ThermostatEntity(GroupEntity, ClimateEntity):
    ...
    async def async_added_to_hass(self):
        ...
        self.async_on_remove(
            async_track_state_change_event(
                self.hass, self._sensor_id, self._async_temperature_sensor_state_change
            )
        )
    ...
    async def _async_temperature_sensor_state_change(
        ...
        _LOGGER.info(
            'Setting current temperature of "%s" to %.1f %s...',
            self._attr_name,
            number_state,
    ...
    async def _async_call_service_action(self, name: str, data: dict[str, Any]):
