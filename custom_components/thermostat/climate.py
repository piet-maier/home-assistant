...
class ThermostatEntity(GroupEntity, ClimateEntity):
    ...
    async def async_set_hvac_mode(self, hvac_mode: HVACMode):
        ...
        await self._async_call_service(SERVICE_SET_HVAC_MODE, data)

    async def async_turn_on(self):
        data = {ATTR_HVAC_MODE: HVACMode.HEAT}

        await self._async_call_service(SERVICE_TURN_ON, data)

    async def async_turn_off(self):
        data = {ATTR_HVAC_MODE: HVACMode.OFF}

        await self._async_call_service(SERVICE_TURN_OFF, data)
    ...
    async def async_set_temperature(self, **kwargs: Any):
        ...
        await self._async_call_service(SERVICE_SET_TEMPERATURE, data)

    async def _async_call_service(self, name: str, data: dict[str, Any]):
