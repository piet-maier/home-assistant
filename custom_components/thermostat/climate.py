...
from .const import TEMPERATURE_SENSOR
...
class ThermostatEntity(GroupEntity, ClimateEntity):
    ...
    def __init__(
        ...
        self._temperature_sensor_id = sensor

    async def async_added_to_hass(self):
        await super().async_added_to_hass()

        if self._temperature_sensor_id is None:
            return
        ...
        self.async_on_remove(
            async_track_state_change_event(
                ...
                self._async_temperature_sensor_state_change,
            )
        )

    @callback
    async def _async_temperature_sensor_state_change(
        ...
        number_state = float(sensor_state)
        ...
        for group_entity in map(registry.async_get, self._entity_ids):
            ...
            for entity in registry.entities.values():
                ...
                await self.hass.services.async_call(
                    NUMBER_DOMAIN,
                    SERVICE_SET_VALUE,
                    {ATTR_ENTITY_ID: entity.entity_id, ATTR_VALUE: number_state},
                    True,
                    self._context,
                )
